...
"=============================================================================
let g:oreo#o#lim#silo#rootdir = get(g:, 'oreo#o#lim#silo#rootdir', '~/.config/silo/'. matchstr(expand('<sfile>'), 'autoload/\zs.\{-1,}\ze/'))
let s:SEP = "\<C-k>\<Tab>"
...
function! s:Builder['_termsamples_'.s:TYPE_STR](records, fieldidx) "{{{
  ...
  try
    let ret = oreo#o#lim#misc#uniq(ret)
  catch /E117/
...
"Public:
let s:Silo = {}
function! oreo#o#lim#silo#newSilo(name, fields, ...) abort "{{{
  let funcopt = a:0 ? a:1 : {}
  let obj = copy(s:Silo)
  let obj.key = get(funcopt, 'key', '')
  let obj.path = a:name=~'[/\\]' ? a:name : expand(g:oreo#o#lim#silo#rootdir).'/'.a:name
  let obj.dir = fnamemodify(obj.path, ':h')
  if s:_is_invalid_fields(a:fields)
    echoerr 'oreo#o#lim#silo#newSilo(): invalid fields > '. string(a:fields)
  end
...
function! s:Silo.select_distinct(where, ...) "{{{
  ...
  try
    ...
    let ret = oreo#o#lim#misc#uniq(records)
    return ret
  catch /silo: invalid \%(condition\|format\)/
...
function! s:Silo.commit(...) "{{{
  ...
  if a:0
    let rollbackpath = a:1=~'[/\\]' ? a:1 : expand(g:oreo#o#lim#silo#rootdir). '/backup/'. a:1
